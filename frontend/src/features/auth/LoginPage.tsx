  ...
  return (
    <div className="min-h-screen" style={{ background: '#0a0a0f' }}>
      <header className="h-14 border-b flex items-center justify-between px-6" style={{ borderColor: 'var(--border)' }}>
        ...
        <Link to="/register" className="text-sm text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors font-medium">Create Account</Link>
      </header>
      ...
      <div className="flex items-center justify-center p-4 pt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-[var(--text-muted)]">Sign in to your dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '32px' }}>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input type={showPw ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required className="input-field pl-10 pr-10" />
